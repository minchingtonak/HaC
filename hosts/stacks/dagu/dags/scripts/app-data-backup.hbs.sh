...
function backup() {
    ...
    # TODO refactor into single script, the logic below is the only unique code
    # can take appdata|personalfiles as first arg and run correct backup procedure

    echo '📸 beginning app data backup'
    ...
    # Run single backup command with all folders
    if [ ${#backup_args[@]} -gt 0 ]; then
        echo "Running backup for ${#backup_args[@]} folders"

        # shellcheck disable=SC2016
        export PBS_ENCRYPTION_PASSWORD='{{{SECRET_APPDATA_ENCRYPTION_PASSWORD}}}'
        ...
        proxmox-backup-client backup \
            "${backup_args[@]}" \
            --repository 'root@pam@192.168.0.189:appdata' \
            --change-detection-mode=metadata \
            --keyfile ./keys/appdata.key # --dry-run
        { set +x; } 2>/dev/null
        ...
        echo "No folders found to backup"
    fi

    curl -fsS -m 10 --retry 5 -o /dev/null "{{{SECRET_APPDATA_HEALTHCHECK_PING_URL}}}"

    echo '✅ app data backup completed successfully!'
}
